...
namespace tokenizer {
    ...
    GPT2Tokenizer::GPT2Tokenizer(const std::string &vocab_file, const std::string &merges_file,
                                 const size_t &vocab_size, const size_t &n_special, const std::wstring &unk_token,
                                 const std::wstring &bos_token, const std::wstring &eos_token,
                                 const std::wstring &pad_token, const bool &add_prefix_space,
    ...
    std::vector<std::wstring> GPT2Tokenizer::bpe(const std::wstring &token) {
...

        {
            ...
            while (true) {
                ...
                ranks.push_back(m_bpe_ranks.find({token.substr(i, length), token.substr(i + length, next_length)}));
        ...
        while (true) {
            const auto bigram =
                std::min_element(ranks.begin(), ranks.end(), [this](const auto &lhs, const auto &rhs) -> bool {
    ...
    std::unordered_set<std::pair<std::wstring, std::wstring>, pair_hash>
    GPT2Tokenizer::get_pairs(const std::vector<std::wstring> &words) {
